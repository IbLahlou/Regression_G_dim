...
# Le process prends bq de temps pour le bons choix des paramétre c'est pour cela que j'ai opté pour le grid search une methode moins 
bparams <- grid_search_elastic_net(X_tr, y_tr, lambda1, lambda2, iter)
blambda1 <- bparams$lambda1
blambda2 <- bparams$lambda2
beta_elnet_gs <- bparams$beta_elastic_net

mse_elnet_gs <- sum((y_ts - X_ts %*% beta_elnet_gs)^2) / nrow(X_ts)
acc_elnet_gs <- ols_accuracy(y_ts, X_ts %*% beta_elnet_gs)

# Print the results
cat("Optimal lambda1:", blambda1, "\n")
cat("Optimal lambda2:", blambda2, "\n")
cat("Mean Squared error:", mse_elnet_gs, "\n")
cat("Accuracy_elnet_gs:", acc_elnet_gs, "\n")
cat("Optimal beta_elnet_gs:", beta_elnet_gs, "\n")
